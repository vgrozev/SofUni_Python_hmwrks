...
for male in [A for A in people if A['gender'] == 'male']:
    for female in [B for B in people if B['gender'] == 'female']:
        ...
        correct_age_diff= abs(int(male['age']) - int(female['age'])) <= 6
        correct_ex_match = male['name'] not in female['ex'] and female['name'] not in male['ex']

        if match and correct_age_diff and correct_ex_match:
            print("{} ({}) and {} ({}); общи интереси: {}".format(
                male['name'], str(male['age']),
                female['name'], str(female['age']),
                ', '.join(match)
            ))
...
# for male in [A for A in people if A['gender'] == 'male']:
#     for female in [B for B in people if B['gender'] == 'female']:
#         if (set(male['interests']).intersection(set(female['interests']))) and (abs(int(male['age']) - int(female['age'])) <= 6) and (male['name'] not in female['ex'] and female['name'] not in male['ex']):
#             print("{} ({}) and {} ({}); общи интереси: {}".format(male['name'], str(male['age']), female['name'], str(female['age']), ', '.join(set(male['interests']).intersection(set(female['interests'])))))
#
